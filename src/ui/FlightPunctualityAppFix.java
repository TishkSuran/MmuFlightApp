 ...
 * This version includes fixes for the Analysis Panel issues.
 */
public class FlightPunctualityAppFix extends JFrame {
     ...
     */
    public FlightPunctualityAppFix() throws SQLException {
    ...
    private void configureUI() {
        ...
        // Right panel: detail panel and analysis panel
        JPanel rightPanel = new JPanel(new BorderLayout());

        // FIX: Explicitly set preferred size for analysis panel
        analysisPanel.setPreferredSize(new Dimension(400, 500));

        // FIX: Add analysis panel to a scroll pane for better visibility
        JScrollPane analysisScrollPane = new JScrollPane(analysisPanel);

        // FIX: Use a vertical split pane for the right panel
        JSplitPane rightSplitPane = new JSplitPane(JSplitPane.VERTICAL_SPLIT);
        rightSplitPane.setTopComponent(detailPanel);
        rightSplitPane.setBottomComponent(analysisScrollPane);
        ...
        // FIX: Add direct test menu items for debugging
        analysisMenu.addSeparator();

        JMenuItem testAirlineItem = new JMenuItem("Test Airline Chart");
        testAirlineItem.addActionListener(e -> {
            // Create simple test data
            Map<String, Double> testData = new java.util.HashMap<>();
            testData.put("American Airlines", 25.5);
            testData.put("Delta Air Lines", 18.2);
            testData.put("United Airlines", 22.7);
            testData.put("Southwest Airlines", 15.3);

            // Display chart with test data
            analysisPanel.showAirlineDelayChart(testData, 2023);
            statusLabel.setText("Test airline chart displayed");
        });
        analysisMenu.add(testAirlineItem);
    ...
    private void handleAirlineAnalysis(ActionEvent e) {
        ...
        if (yearStr != null && !yearStr.trim().isEmpty()) {
            try {
                ...
                // FIX: Add debugging
                System.out.println("Airline data: " + data.size() + " airlines found");
                ...
                // FIX: Force repaint and revalidate
                analysisPanel.revalidate();
                analysisPanel.repaint();

                // FIX: Make sure the analysis panel is visible
                ...
                statusLabel.setText("Analysis failed with error: " + ex.getMessage());
            } finally {
                setCursor(Cursor.getDefaultCursor());
            }
        }
    ...
    private void handleAirportAnalysis(ActionEvent e) {
        ...
        if (yearStr != null && !yearStr.trim().isEmpty()) {
            try {
                ...
                // FIX: Add debugging
                System.out.println("Airport data: " + data.size() + " airports found");
    ...
    private void handleTimeSeriesAnalysis(ActionEvent e) {
        ...
        try {
            ...
            if (selectedAirport != null && !selectedAirport.trim().isEmpty()) {
                ...
                // FIX: Add debugging
                System.out.println("Time series data: " + data.size() + " months found");
    ...
    public static void main(String[] args) {
        ...
        SwingUtilities.invokeLater(() -> {
            try {
                FlightPunctualityAppFix app = new FlightPunctualityAppFix();
                app.setVisible(true);
